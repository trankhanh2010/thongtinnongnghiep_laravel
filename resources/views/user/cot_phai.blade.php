{{-- <div class="col-md-2 cot_phai">
    <div class="bg-white p-3 sticky-top">
        @if ($ds_tintuc_noibat != null)
            <h4>Tin tức</h4>
            @foreach ($ds_tintuc_noibat as $key => $item)
...
<div class="col-md-3  cot_trai ">
    <div class="p-3 sticky-top bg-white  " >
            <div class="">
                <div class=" p-2">
                  <h4 class="fw-bold">Các giống lúa</h4>
                  @if($ds_cac_gionglua !=null)
                  @foreach ($ds_cac_gionglua as $key => $item)

                  {{-- <a href="#" class=" link-underline-opacity-0 link-primary d-block p-2 ">{{ $item->ten }}</a>  --}}
                  <a href="{{route('gionglua',['ma'=> $item->ma])}}" class=" link-underline-opacity-0 link-secondary d-block p-2 fw-bold">{{ $item->ten }}</a> 
