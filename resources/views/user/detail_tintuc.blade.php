...
@section('content')
        ...
        <div>
            ...
            <div class="hinhanh_detail">
                @foreach ($ds_hinhanh_detail as $key => $item)
                    <img src="/hinhanh/hinhanh_tintuc/{{ $item->ten }}" class="img-thumbnail" alt="..."
                        style="width:100% ; height: 400px">
                @endforeach
            </div>
            <div class="ds_hinhanh_detail border border-5">
                @foreach ($ds_hinhanh_detail as $key => $item)
                    <img src="/hinhanh/hinhanh_tintuc/{{ $item->ten }}" class="img-thumbnail border border-5"
    ...
    <hr>
    @if (count($detail[0]->loai_tintuc) > 0)
        <h4 class="fw-bold">Loại tin tức</h4>
        @foreach ($detail[0]->loai_tintuc as $item2)
        <a href="{{route('ds_tintuc',['key_ds_loai_tintuc'=>$item2->ten_tintuc.','])}}"><span class="badge text-bg-warning">{{ $item2->ten_tintuc }}</span></a>
        @endforeach
    @endif
    @if (count($detail[0]->gionglua_lienquan) > 0)
    <h4 class="fw-bold">Giống lúa liên quan</h4>
    @foreach ($detail[0]->gionglua_lienquan as $item2)
       <a href="{{route('gionglua',['ma'=> $item2->ma])}}"> <span class="badge text-bg-success">{{ $item2->ten_gionglua_lienquan }}</span></a>
    @endforeach
@endif
        ...
        <form id ="ajaxform" class="add">
            <div class="panel">
                <div class="panel-body">
                    <input type="text" class="d-none" value="{{ $detail[0]->ma }}" name="ma_tintuc" id="ma_tintuc">
...
@section('js')
    <script>
        ...
        var ENDPOINT = "{{ route('tintuc', ['ma' => $detail[0]->ma]) }}";
        var page = 1;
        ...
        $('#ajaxform').on('submit', function(event) {
            ...
            noidung = $('#noidung').val();
            ma_user = $('#ma_user').val();
            ma_tintuc = $('#ma_tintuc').val();


            $.ajax({
                url: "{{ route('binhluan_tintuc', ['ma' => $detail[0]->ma]) }}", //Define Post URL
                type: "POST",
                data: {
                    noidung: noidung,
                    ma_user: ma_user,
