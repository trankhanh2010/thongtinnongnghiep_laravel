...
<br>
        ...
        <div>
            ...
            <div class="hinhanh_detail">
                @foreach ($ds_hinhanh_detail as $key => $item)
                    <img src="/hinhanh/hinhanh_saubenh/{{ $item->ten }}" class="img-thumbnail" alt="..."
                        style="width:100% ; height: 400px">
                @endforeach
            </div>
            <div class="ds_hinhanh_detail border border-5">
                @foreach ($ds_hinhanh_detail as $key => $item)
                    <img src="/hinhanh/hinhanh_saubenh/{{ $item->ten }}" class="img-thumbnail border border-5" alt="..."
    ...
    <hr>
    <div>
        <h4>Các giống lúa chống chịu tốt với sâu bệnh này</h4>
    </div>
        ...
        @foreach ($ds_chongchiutot as $item)
            <div class="col border border-5">
                <div class="card zoom" style="height: 405px">
                    <img src="/hinhanh/hinhanh_gionglua/{{ $item->ten_hinhanh }}" class="card-img-top" alt="..."
                    ...
                    <div class="card-body overflow-hidden" style="height: 160px">
                        ...
                        <a href="{{ route('gionglua', ['ma' => $item->ma_gionglua]) }}" class="stretched-link"></a>
                    </div>
                </div>
            </div>
        @endforeach
    ...
    <hr>
    <div>
        <h4>Các giống lúa dễ bị nhiễm bởi sâu bệnh này</h4>
    </div>
    ...
    <form id ="ajaxform" class="add">
        <div class="panel">
            <div class="panel-body">
                <input type="text" class="d-none" value="{{ $detail[0]->ma }}" name="ma_saubenh" id="ma_saubenh">
...
@section('js')
    <script>
        ...
        var ENDPOINT = "{{ route('saubenh', ['ma' => $detail[0]->ma]) }}";
        var page = 1;
        ...
        $('#ajaxform').on('submit', function(event) {
            ...
            noidung = $('#noidung').val();
            ma_user = $('#ma_user').val();
            ma_saubenh = $('#ma_saubenh').val();


            $.ajax({
                url: "{{ route('binhluan_saubenh', ['ma' => $detail[0]->ma]) }}", //Define Post URL
                type: "POST",
                data: {
                    noidung: noidung,
                    ma_user: ma_user,
                    ma_saubenh: ma_saubenh
